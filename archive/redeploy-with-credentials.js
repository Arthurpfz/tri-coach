const axios = require('axios');
const fs = require('fs');
require('dotenv').config();

const N8N_API_KEY = process.env.N8N_API_KEY;
const N8N_BASE_URL = 'https://apfz.app.n8n.cloud/api/v1';
const OLD_ID = 'SpVk2Ft3xDAaSZvY';

const client = axios.create({
  baseURL: N8N_BASE_URL,
  headers: {
    'X-N8N-API-KEY': N8N_API_KEY,
    'Accept': 'application/json',
    'Content-Type': 'application/json'
  }
});

async function redeploy() {
  console.log('Redeploying with correct credentials...\n');

  try {
    // Delete old version
    await client.delete(`/workflows/${OLD_ID}`);
    console.log('✅ Deleted old version');
    
    // Create new version with correct credential ID
    const workflow = JSON.parse(fs.readFileSync('intervals-icu-workflow.json', 'utf8'));
    const response = await client.post('/workflows', workflow);
    
    console.log('✅ Deployed with credentials!');
    console.log('   ID:', response.data.id);
    console.log('   Name:', response.data.name);
    console.log('\n🚀 Ready to activate:');
    console.log('   https://apfz.app.n8n.cloud/workflow/' + response.data.id);
    console.log('\nClick "Inactive" toggle to activate.\n');
    
    // Open in browser
    require('child_process').exec(`open "https://apfz.app.n8n.cloud/workflow/${response.data.id}"`);
    
  } catch (error) {
    console.error('❌ Error:', error.response?.data || error.message);
  }
}

redeploy();
